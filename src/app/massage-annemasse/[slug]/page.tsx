...
import { ANNEMASSE_BASE_PATH } from "@/config/annemasse";
import {
...
const BRAND_NAME = "Marina Masaje";
...
export async function generateMetadata({
  ...
  if (!massage) return { title: BRAND_NAME };
  ...
  const ogTitle = `${title} | ${BRAND_NAME}`;
...
export default async function AnnemasseMassageCategoryPage({
  params,
  searchParams,
}: {
  params: Promise<{ slug: string }>;
  ...
  const backHref = locale !== marinaDefaultLocale ? `${ANNEMASSE_BASE_PATH}?lang=${locale}` : ANNEMASSE_BASE_PATH;
          ...
          <div className="space-y-6">
            <div className="flex items-start justify-between gap-3">
              <Link
                ...
                aria-label={`${backLabel} ${BRAND_NAME}`}
              >
