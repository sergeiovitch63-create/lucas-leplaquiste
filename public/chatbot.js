...
(function () {
  ...
  if (window._fcInitialized) return;
...
2. Debes hacer EXACTAMENTE 7 preguntas en total antes de hacer recomendaciones
3. Haz UNA pregunta a la vez, esperando la respuesta antes de continuar
4. Las preguntas deben cubrir: (1) ocasión/uso/regalo, (2) preferencias de sabor (picante/dulce/salado), (3) restricciones dietéticas (gluten, vegano), (4) tipo de producto preferido (comida/bebida/postre), (5) intensidad de sabor deseada, (6) ocasión especial o persona destinataria, (7) presupuesto o formato (pack/individual)
5. IMPORTANTE: Cuenta el número de respuestas del usuario. Solo después de recibir EXACTAMENTE 7 respuestas, puedes hacer recomendaciones
6. En cada mensaje, indica cuántas preguntas has hecho hasta ahora (ej: "Pregunta 1 de 7", "Pregunta 2 de 7", etc.)
7. Solo después de las 7 preguntas respondidas, propón exactamente 2 o 3 productos
8. AL FINAL de tu recomendación añade OBLIGATORIAMENTE: [PRODUCTOS_RECOMENDADOS: ID1, ID2, ID3]
9. NUNCA hagas recomendaciones antes de tener 7 respuestas del usuario
...
- Siempre termina con [PRODUCTOS_RECOMENDADOS: X, X, X] cuando hagas recomendación final`;
  ...
  #fc-bub{position:fixed;bottom:28px;right:106px;z-index:9997;background:linear-gradient(135deg,#c9963a,#e8b85a);color:#1a0f08;font-family:'Lato',sans-serif;font-size:.77rem;font-weight:700;padding:8px 14px;border-radius:20px 4px 20px 20px;white-space:nowrap;pointer-events:none;box-shadow:0 4px 16px rgba(201,150,58,.4);animation:fc-bi .4s ease 1.8s both;transition:opacity .3s,transform .3s;max-width:200px;}
  #fc-bub::after{content:'';position:absolute;right:-8px;top:50%;transform:translateY(-50%);width:0;height:0;border-top:8px solid transparent;border-bottom:8px solid transparent;border-left:8px solid #e8b85a;filter:drop-shadow(2px 0 2px rgba(0,0,0,.1));}
  #fc-bub.hidden{opacity:0;transform:scale(.85);pointer-events:none;}
  @keyframes fc-bi{from{opacity:0;transform:translateY(8px) scale(.9)}to{opacity:1;transform:translateY(0) scale(1)}}
  @media(max-width:480px){#fc-bub{right:86px;font-size:.7rem;padding:6px 12px;max-width:160px;}}
  ...
  // ── BUBBLE TEXT BY LANGUAGE ──
  const getBubbleText = () => {
    const lg = (typeof window.lang !== 'undefined' ? window.lang : 'es');
    const texts = {
      ...
      pl: 'Jak mogę ci pomóc? 👋'
    };
    return texts[lg] || texts.es;
  };

  // ── DOM ──
  const bub = document.createElement('div');
  bub.id = 'fc-bub'; bub.textContent = getBubbleText();
  ...
  // ── QUICK REPLIES (removed, now using question options) ──
  ...
  function resetChat() {
    ...
    document.getElementById('fc-msgs').innerHTML = '';
  ...
  btn.addEventListener('click', () => {
    ...
    if (isOpen) {
      if (!messages.length) {
        ...
        const lg = (typeof window.lang !== 'undefined' ? window.lang : 'es');
        const greetings = {
          es: '¡Hola! 👋 Soy el asistente de **Fincas Canarias**. Te haré 7 preguntas para encontrar los productos perfectos para ti.',
          en: 'Hello! 👋 I\'m the **Fincas Canarias** assistant. I\'ll ask you 7 questions to find the perfect products for you.',
          de: 'Hallo! 👋 Ich bin der Assistent von **Fincas Canarias**. Ich stelle Ihnen 7 Fragen, um die perfekten Produkte für Sie zu finden.',
          fr: 'Bonjour ! 👋 Je suis l\'assistant de **Fincas Canarias**. Je vais vous poser 7 questions pour trouver les produits parfaits pour vous.',
          it: 'Ciao! 👋 Sono l\'assistente di **Fincas Canarias**. Ti farò 7 domande per trovare i prodotti perfetti per te.',
          ru: 'Привет! 👋 Я помощник **Fincas Canarias**. Я задам вам 7 вопросов, чтобы найти идеальные продукты для вас.',
          pl: 'Cześć! 👋 Jestem asystentem **Fincas Canarias**. Zadam Ci 7 pytań, aby znaleźć idealne produkty dla Ciebie.'
        };
        setTimeout(() => {
          addBot(greetings[lg] || greetings.es);
          showQuestion(0);
        }, 360);
      }
  ...
  // Update bubble text when language changes
  const updateBubbleText = () => {
    if (bub && !bub.classList.contains('hidden')) {
      bub.textContent = getBubbleText();
    }
  };
  
  // Watch for language changes
  let lastLang = (typeof window.lang !== 'undefined' ? window.lang : 'es');
  setInterval(() => {
    const currentLang = (typeof window.lang !== 'undefined' ? window.lang : 'es');
  ...
  function showCards(ids) {
    ...
    title.className = 'fc-cards-title';
    const titles = {
      es: '✦ Mis recomendaciones para ti',
      en: '✦ My recommendations for you',
      de: '✦ Meine Empfehlungen für Sie',
      fr: '✦ Mes recommandations pour vous',
      it: '✦ I miei consigli per te',
      ru: '✦ Мои рекомендации для вас',
      pl: '✦ Moje rekomendacje dla Ciebie'
    };
    title.textContent = titles[lg] || titles.es;
    ...
    prods.forEach(p => {
      ...
      card.onclick = () => { if (typeof window.openModal==='function') { btn.click(); setTimeout(()=>window.openModal(p.id),320); } };
      card.innerHTML = `
  ...
  // ── GET QUESTIONS FROM WINDOW ──
  function getQuestions() {
    return window.CHATBOT_QUESTIONS || [];
  }

  // ── GET RECOMMENDATIONS ──
  function getRecommendations(answers, lang) {
    if (typeof window.getRecommendedProducts === 'function') {
      return window.getRecommendedProducts(answers, lang, 3);
    }
    return [];
  }
  ...
  function showQuestion(questionIndex) {
    ...
    const lg = (typeof window.lang !== 'undefined' ? window.lang : 'es');
    const questionText = question.question[lg] || question.question.es;

    setTimeout(() => {
      addBot(questionText);
      const qrEl = document.getElementById('fc-qr');
      qrEl.innerHTML = '';
      question.options.forEach((opt) => {
        const btn = document.createElement('button');
        btn.className = 'fc-qb';
        btn.textContent = opt.label[lg] || opt.label.es;
        btn.addEventListener('click', () => {
          qrEl.innerHTML = '';
          send(opt.label[lg] || opt.label.es);
        });
  ...
  async function send(text) {
    ...
    setTimeout(() => {
      ...
      if (userAnswers.length < questions.length) {
        ...
        const recommendedProducts = getRecommendations(userAnswers, lg);
        const recommendationTexts = {
          es: '¡Perfecto! Basándome en tus respuestas, te recomiendo estos productos:',
          en: 'Perfect! Based on your answers, I recommend these products:',
          de: 'Perfekt! Basierend auf Ihren Antworten empfehle ich diese Produkte:',
          fr: 'Parfait ! Basé sur vos réponses, je vous recommande ces produits :',
          it: 'Perfetto! Basandomi sulle tue risposte, ti consiglio questi prodotti:',
          ru: 'Отлично! Основываясь на ваших ответах, я рекомендую эти продукты:',
          pl: 'Świetnie! Na podstawie Twoich odpowiedzi polecam te produkty:'
        };
        addBot(recommendationTexts[lg] || recommendationTexts.es);
        
        if (recommendedProducts.length > 0) {
          const productIds = recommendedProducts.map(p => p.id);
          setTimeout(() => showCards(productIds), 420);
        } else {
          // Fallback recommendations
          setTimeout(() => showCards([16, 17, 26]), 420);
        }
      }
